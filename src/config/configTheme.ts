...
export const getSanitizedConfig = (config: Config): SanitizedConfig | Record<string, never> => {
  try {
    return {
      github: {
      ...
      experiences:
        config?.experiences?.filter((experience) => experience.company || experience.position || experience.from || experience.to) || [],
      certifications:
        config?.certifications?.filter((certification) => certification.year || certification.name || certification.body) || [],
      educations: config?.educations?.filter((item) => item.institution || item.degree || item.from || item.to) || [],
...
export const skeleton = ({
  ...
  return React.createElement('div', { className: classNames.join(' '), style });
};
