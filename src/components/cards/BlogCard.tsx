...
import { SanitizedBlog, Article } from '@/types';

const BlogCard = ({ loading, blog, googleAnalyticsId }: { loading: boolean; blog: SanitizedBlog; googleAnalyticsId?: string }) => {
    ...
    const fetchArticles = async () => {
      try {
        if (blog.source === 'medium') {
          const res = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@${blog.username}`);
          ...
          const parsed: Article[] = data.items.map((item: any) => ({
            ...
            link: item.link,
            thumbnail: item.thumbnail || '/fallback.jpg',
          ...
          const res = await fetch(`https://dev.to/api/articles?username=${blog.username}`);
          const data = await res.json();
          ...
          const parsed: Article[] = data.map((item: any) => ({
            ...
            description: item.description,
            link: item.url,
            thumbnail: item.cover_image || '/fallback.jpg',
              ...
              <div className="w-full">
                <div className="flex items-start px-4">
                  <div className="text-center md:text-left w-full">
                    ...
                    <p className="mt-3 text-base-content text-sm">{article.description}</p>
                    <div className="mt-4 flex items-center flex-wrap justify-center md:justify-start">
    ...
    <div className="col-span-1 lg:col-span-2">
      <div className="card bg-base-200 shadow-xl border border-base-300">
        <div className="card-body p-8">
          ...
          <div className="grid grid-cols-1 gap-6">{loading || !articles ? renderSkeleton() : renderArticles()}</div>
        </div>
      </div>
    </div>
  );
